...
public final class CardCollectionViewLayout: UICollectionViewFlowLayout {
    
    private var firstSetupDone = false
    
    private let options: CardCollectionViewLayoutOptions
    ...
    public init(options: CardCollectionViewLayoutOptions = CardCollectionViewLayoutOptions()) {
        self.options = options
        super.init()
    }
    
    public required init?(coder aDecoder: NSCoder) {
        self.options = CardCollectionViewLayoutOptions()
    ...
    private func setup() {
        ...
        itemSize = options.itemSize ?? CGSize(width: collectionView.bounds.width - 150, height: collectionView.bounds.height)
    ...
    public override class var layoutAttributesClass: AnyClass {
        return CardLayoutAttributes.self
    }
    ...
    public override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        for attributes in allAttributes {
            ...
            guard let attributes = attributes as? CardLayoutAttributes else { continue }
